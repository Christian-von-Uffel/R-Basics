...
track earns a Statement of Accomplishment or Verified Certificate with Distinction.
    ...
    Required grade on final: greater than
    q = ((11/12 + 12/12 + 14/15 + 13/14 + 13/13 + 10/10)/6); l = 1; mdtm= .9; prjt = (27/65)
    (80-(q*20+l*15+mdtm*20+prjt*15+12))/15
...
Without Distinction    
    ...
    Required grade on final: greater than
    (80-(q*30+mdtm*30+16))/20
    
    quiz<-c(11/12,12/12,14/15,13/14,11/12,13/13,10/10)
    midterm<-27/30
    labs<-c(100,100,100,100,100,100,100)
    project<-27/65
    final<-25/30

If you have missed a quiz or a lab, please give the value as zero, so that the length of quiz and labs is 7. 

Now copy paste the entire function given below to see your scores:
...
This means that assuming you score a perfect grade on both the quizzes and the labs, you will need to score above 70 for the remaining average total of points for the course

Grades

QS  <- .20 * (11/12 + 12/12 + 14/15)/3
L   <- .15 * (1 + 1 + 1 + 1 + 1 + 1)/6
MT  <- .20 * (13/15 + 14/15)/2
P   <- .15 * (.63)
FNL <- .3  * (.63)

FinalGrade <- QS + L + MT + P + FNL; FinalGrade
...
qnorm(.2, mean = 77, sd=5)
 ...
 Suppose we randomly select four individuals four an experiment in which the probability 
 of success is .35. What is the prob that exactly 1 will be successful?
 Well this is the  of successes: n!/(k!(n-k)!)
...
```
 ...
 Calulate the probability of have 70 or more intances of success in a smaple size 245 
 where the probability of success is .25
